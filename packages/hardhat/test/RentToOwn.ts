  ...
  describe("Airdrop functionality", function () {
    ...
    beforeEach(async function () {
      ...
      const tx = await rentToOwn
        .connect(lender)
        .registerAirdrop(await mockToken.getAddress(), amount, merkleRoot, duration);
    ...
    it("Should register new airdrop", async function () {
      const airdrop = await rentToOwn.airdrops(airdropId);
    ...
    it("Should allow borrower to claim airdrop", async function () {
      ...
      await rentToOwn.connect(borrower).claimAirdrop(0n, airdropId, merkleProof);
    ...
    it("Should prevent double claiming", async function () {
      await rentToOwn.connect(borrower).claimAirdrop(0n, airdropId, merkleProof);
      await expect(rentToOwn.connect(borrower).claimAirdrop(0n, airdropId, merkleProof)).to.be.revertedWith(
        "Airdrop already claimed",
      );
    });

    it("Should prevent non-borrowers from claiming", async function () {
      await expect(rentToOwn.connect(lender).claimAirdrop(0n, airdropId, merkleProof)).to.be.revertedWith(
        "Not the borrower",
      );
    });
  });
